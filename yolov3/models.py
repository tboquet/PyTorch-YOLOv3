...
def create_modules(module_defs, freeze=False):
    """
    ...
    start_freeze = len(module_defs) - 2
    for i, module_def in enumerate(module_defs):
        ...
        if freeze is True and module_def['type'] != 'yolo' and i < start_freeze:
            for module in modules:
                print('Freezing: {}'.format(module))
...
class YOLOLayer(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        grid_y = torch.linspace(0, g_dim - 1, g_dim).repeat(
            g_dim, 1).t().repeat(bs * self.num_anchors, 1,
                                 1).view(y.shape).type(FloatTensor)
        scaled_anchors = [(a_w / stride, a_h / stride)
                          for a_w, a_h in self.anchors]
        anchor_w = FloatTensor(scaled_anchors).index_select(1, LongTensor([0]))
        anchor_h = FloatTensor(scaled_anchors).index_select(1, LongTensor([1]))
        anchor_w = anchor_w.repeat(bs, 1).repeat(1, 1, g_dim *
                                                 g_dim).view(w.shape)
        anchor_h = anchor_h.repeat(bs, 1).repeat(1, 1, g_dim *
                                                 g_dim).view(h.shape)
        ...
        if targets is not None:
            ...
            cls_mask = Variable(
                mask.unsqueeze(-1).repeat(1, 1, 1, 1, self.num_classes).type(
                    FloatTensor))
...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path, img_size=416, freeze=False):
    ...
    def load_weights(self, weights_path):
        ...
        header = np.fromfile(
            fp, dtype=np.int32, count=5)  # First five are header values
